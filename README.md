# 👋 Hi, I'm Carlos Palmieri (Palmi)

🚀 Web Developer focused on building real-world applications with clean UI and solid backend logic.

---

## ⚡ About Me

- 🥪 Building a **custom sandwich builder app** with dynamic pricing  
- 📦 Creating a **cupboard / inventory management system**  
- 💡 Focused on **practical apps with business value**  
- 🌎 Open to **remote opportunities (LATAM / US-friendly)**  

---

## 🧠 Tech Stack

### 🎨 Frontend
<p>
  <img src="https://skillicons.dev/icons?i=html,css,js,react,vite" />
</p>

### 🔥 Backend / BaaS
<p>
  <img src="https://skillicons.dev/icons?i=firebase" />
</p>

### 🛠 Tools
<p>
  <img src="https://skillicons.dev/icons?i=git,github,postman" />
</p>

### 🐍 Other
<p>
  <img src="https://skillicons.dev/icons?i=python" />
</p>

---

## 🚀 Featured Projects

### 🥪 Sandwich Builder App
- Dynamic product builder  
- Real-time pricing  
- Firebase integration  
- Customer-facing UI  

### 📦 Cupboard App
- Inventory tracking  
- Product organization  
- Mobile-friendly design  

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=TU-USUARIO&show_icons=true&theme=tokyonight" height="150"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=TU-USUARIO&layout=compact&theme=tokyonight" height="150"/>
</p>

---

## 🔥 Activity Graph

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=TU-USUARIO&theme=tokyo-night" />
</p>

---

## 🎯 Current Focus

- Building full-stack apps with **real business logic**  
- Improving **UI/UX and user flows**  
- Strengthening **Firebase + data handling**  

---

## 📬 Let's Connect

💼 Open to:
- Web Development roles  
- Technical / Hybrid roles  
- Product-focused teams  

---

⭐ *Check out my repositories to see real projects in action*
